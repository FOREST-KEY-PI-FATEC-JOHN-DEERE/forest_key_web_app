...
export default function RegisterForm({ onSwitchToLogin, noWrapper = false }: AuthFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (password !== confirmPassword) {
      const msg = t('register_error_password_mismatch', 'Senhas não coincidem.');
      setError(msg);
      showError(msg, t('error', 'Erro'));
      setLoading(false);
      return;
    ...
    try {
      ...
      showSuccess(t('register_success', 'Cadastro realizado! Confirme seu e-mail.'), t('success', 'Sucesso'));

      setTimeout(onSwitchToLogin, 900);

    } catch {
      showError(t('register_error_unexpected', 'Erro inesperado. Tente novamente.'), t('error', 'Erro'));
    }
  ...
  const formContent = (
    <>
      <div className="text-center mb-6">
        ...
        <p className="text-sm text-gray-500 dark:text-gray-300">{t('register_subtitle', 'Preencha os dados para criar sua conta')}</p>
      </div>

      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="text-xs text-gray-700 dark:text-gray-200">{t('first_name', 'Nome')}</label>
          ...
          <div>
            <label className="text-xs text-gray-700 dark:text-gray-200">{t('last_name', 'Sobrenome')}</label>
        ...
        <div>
          <label className="text-xs text-gray-700 dark:text-gray-200">{t('email', 'E-mail')}</label>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="text-xs text-gray-700 dark:text-gray-200">{t('password', 'Senha')}</label>
          ...
          <div>
            <label className="text-xs text-gray-700 dark:text-gray-200">{t('confirm_password', 'Repita a senha')}</label>
